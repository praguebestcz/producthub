...
* **2026-07-17**
  * M8 (light) - **Prompt z komentářů**: interní tým vybere komentáře → vygeneruje se markdown prompt pro Claude Code → uloží jako „zadání" (nová tabulka `PromptExport` po db-security-expert review) → seznam „Předaná zadání" se stavy (Vytvořeno/Předáno vývoji/Zapracováno), kopie/stažení .md. Zjednodušená verze původního M8 (bez schvalovacího procesu). Detailní spec: outputs/m8-prompt-z-komentaru-spec.md - Hana Ortmannová
