...
* **2026-08-05**
  * **Komentování uvnitř obalených prvků + bublina u kurzoru + celá obrazovka.** (1) Kotva komentáře: `data-review-id` předka se použije, jen když označuje **kompaktní prvek** (do ~25 % plochy okna prohlížeče - tlačítko, pole, karta); velké obaly (sekce, rám diagramu) se přeskočí a kotvou je přímo kliknutý prvek přes DOM cestu. Uvnitř diagramů a dlouhých sekcí tak jde komentovat jednotlivé prvky (dřív celý diagram = jedno vlákno). Hover rámeček nově ukazuje přesně to, co klik vybere. (2) **Bublina nového komentáře se otevírá u místa kliknutí** (ne pod celým prvkem) a je **vždy celá viditelná** - skutečná výška se měří, při malém okně se obsah roluje uvnitř bubliny. (3) Špendlíky a rámeček výběru se srovnávají i při scrollu vnořeného kontejneru (diagram s vlastním posuvníkem). (4) Nové tlačítko **„Celá obrazovka"** v liště prohlížeče (Esc ukončí). (5) Výška rámu prohlížeče se měří místo odhadu - stránka už nedostává falešný posuvník. - Hana Ortmannová
* **2026-07-30**
  * **Prohlížeč specifikace - zklidnění rozložení.** Panel komentářů je nově **sloupec vedle dokumentu**, ne překryv - otevřený panel dokument zúží, takže pravá část specifikace i se svými špendlíky zůstane vidět. Navigace mezi stránkami se přesunula **dovnitř rámu dokumentu** (nad dokumentem tak visí o jeden samostatný pruh méně) a bannery režimu jsou o řádek nižší. **Špendlíky, které by se překryly** (dva komentované prvky blízko pod sebou), se navzájem odsadí. V panelu u vlákna svítí **ikona špendlíku** místo pořadového čísla - špendlík v dokumentu nese avatar autora, takže číslo nemělo k čemu odkazovat. - Hana Ortmannová
...
* **2026-07-27**
  ...
  * **Živý zvoneček** (M7 Fáze 2 dokončení): notifikace se doručují přes SSE stream per uživatel (`/api/notifications/stream`, paměťový hub `notifyUser` po COMMITu) - zvoneček naskočí okamžitě, bez pollování. Pojistný pomalý poll (120 s) + auto-reconnect EventSource. Signál nenese data (klient natáhne `/api/notifications` s vlastní session). Stejné zpevnění jako přítomnost (rate-limit, strop 8 spojení, heartbeat re-auth). - Hana Ortmannová
  * **E-mailové notifikace** (nastavitelné u uživatele: Vypnuto / Okamžitě / Jen nepřečtené): e-maily navazují na zvoneček (stejné události i rozsah ALL/INVOLVED). Okamžité = po COMMITu (fire-and-forget); „jen nepřečtené" = úloha na pozadí (interval 2 min, prodleva 5 min, jen posledních 24 h). Nové `User.emailNotify` (default OFF) + `Notification.emailedAt` (po db-security-expert review: backfill, partikulární index, atomický claim, sdílený re-check viditelnosti/členství/deaktivace, escapování). Odesílání `nodemailer`/SMTP jako vratky (mimo produkci jen na TEST_EMAILS). Detailní spec: outputs/email-notifikace-spec.md - Hana Ortmannová
  * **Proklik ze zvonečku/e-mailu trvale zvýrazní prvek**: po otevření komentáře přes `?comment=` se jeho HTML prvek ve specifikaci zvýrazní TRVALE (zůstane, dokud uživatel neklikne jinam), místo krátkého pulzu, který na čerstvě načtené stránce proběhl dřív, než se uživatel podíval. Highlight se navíc zopakuje (prvek/špendlíky se po načtení dorovnávají) a hláška „prvek nenalezen" se během prokliku potlačí. - Hana Ortmannová
  * **Jednotná barva uživatele**: avatar autora u komentáře má stejný barevný kroužek (`userColor` podle userId) jako v liště přítomnosti - autor je poznat napříč záhlavím dokumentu i komentáři; skutečný avatar se zobrazí, pokud ho uživatel má. - Hana Ortmannová
  * **Špendlík jako Google komentáře**: špendlík u prvku ukazuje avatar autora a - jen u vlákna s víc než jednou zprávou - počet zpráv ve vláknu (místo dřívějšího pořadového čísla, které nemělo význam). - Hana Ortmannová
  * **Zvýraznění patří jen aktivnímu vláknu**: v přehledu všech komentářů (žádné konkrétní vlákno není aktivní) se trvalé zvýraznění prvku zruší - jinak zůstávalo viset po prokliku z e-mailu/zvonečku. - Hana Ortmannová
  * **Akce u vlastního komentáře v kebab menu (⋮)**: Upravit a Smazat u vlastního komentáře/odpovědi jsou nově v kebab menu (vzor Google komentářů) místo dřívějších nenápadných textových odkazů, které šlo přehlédnout; Smazat potvrzuje dialog. Přidáno do styleguide (sekce „Akce u vlastního komentáře"). - Hana Ortmannová
  * **Okno „Co je nového" jen jednou + odznak u Nápovědy**: okno s novinkami vyskočí po nasazení jen JEDNOU (dřív se otevíralo při každém přechodu mezi stránkami, dokud uživatel neklikl „Rozumím"). Nový klíč `WHATSNEW_SHOWN_KEY` hlídá, že se pro danou nejnovější verzi ukázalo. Nepřečtené novinky pak indikuje tečka u položky „Nápověda" v menu; tečka zmizí až po otevření Nápovědy (kde je celý přehled novinek). „Rozumím" v okně už tečku neschovává - okno je jen jednorázové oznámení, tečka zůstane jako připomínka (klíč `LAST_SEEN_KEY` nastavuje jen návštěva Nápovědy, událost `NEWS_SEEN_EVENT` sladí odznak napříč). - Hana Ortmannová
  * **Dev nástroje pro testování**: lokální falešná schránka Mailpit (`docker-compose`, web UI :8025) + dev-only přihlášení testovacího účtu `GET /api/dev/login?user=b|c` (na produkci 404, proxy pouští jen mimo produkci). - Hana Ortmannová
* **2026-07-22**
...
* 🟠 M6 - lišta: výrazný přepínač režimů Procházení / Komentování (jen pro roli Komentátor a vyšší; aktivní Komentování svítí barevně, ať je na první pohled jasné, v jakém režimu jsi).
  ...
  * Kotva výběru: `data-review-id` nejbližšího předka se použije, jen když označuje **kompaktní prvek** (do ~25 % plochy okna prohlížeče - tlačítko, pole, karta). Velký obal (sekce, rám diagramu) výběr NEpřebírá - kotvou je přímo kliknutý prvek (DOM cesta). Uvnitř diagramů tak lze komentovat jednotlivé prvky. Hover ukazuje stejnou kotvu, jakou klik vybere.
  * Klik nebo hover na prázdnou plochu velkého kontejneru (pozadí SVG diagramu, plocha sekce) NEvybírá nic - chová se jako klik do pozadí stránky (zavře rozdělanou bublinu). Obsahové prvky (obrázek, odstavec, tvar diagramu) jdou komentovat i velké.
  * Bublina nového komentáře se otevírá **u místa kliknutí** a je vždy celá viditelná (skutečná výška se měří; v malém okně se obsah bubliny roluje uvnitř). Při scrollu - včetně scrollu vnořeného kontejneru (diagram s vlastním posuvníkem) - drží bublina, rámeček i špendlíky u svého prvku.
* 🟢 Tlačítko **„Celá obrazovka"** v liště prohlížeče: lišta + dokument + panel překryjí celé okno (víc místa na specifikaci), Esc nebo tlačítko vrátí zpět. Řešeno překryvem v aplikaci, ne nativním fullscreen API - dialogy a hlášky zůstávají viditelné. Výška rámu prohlížeče se měří (žádný falešný posuvník stránky).
