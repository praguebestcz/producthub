...
* **Složky klientů (požadavek Hany, 2026-07-10):** entita Klient (DDS, Foto Škoda, Harfasport, … včetně PB) seskupující projekty pod sebou - kopíruje strukturu `clients/{klient}/`. Znamená model `Client` + `Project.clientId` + seskupený dashboard. Navrhované zařazení: **M3.5** (hned po schválení M3, dokud je schéma mladé a migrace levná).
