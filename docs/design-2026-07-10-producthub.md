...
* **XSS ochrana:** iframe `sandbox="allow-scripts allow-forms allow-popups"` BEZ `allow-same-origin` → nahraný dokument běží v opaque originu: jeho JS funguje (modaly, interakce), formuláře i otevírání oken (klikací prototypy je potřebují), ale nemůže číst naše cookies ani volat API jako uživatel. Proto je auth v URL tokenu, ne v cookie. Stejná hodnota i v `Content-Security-Policy: sandbox …` na view response. `allow-forms`/`allow-popups` posouzeny auditem 2026-07-17 (opaque origin drží, session chráněná, dopad nízký). Trade-off: uvnitř prototypu nefunguje localStorage/cookies - přijatelné.
...
* **M9 - Přenos komentářů mezi verzemi + dokončení:** migrace kotev ve DVOU stupních (kvůli klikacím prototypům, kde elementy vznikají až JavaScriptem za běhu a statická analýza je nevidí):
  * *Volitelnost přenosu (přání Hany 2026-07-20):* při nahrání nové verze si autor VYBERE, zda přenést komentáře z předchozí verze, nebo začít s čistou verzí (přepínač v dialogu „Nová verze"; default = přenést). Když zvolí NEpřenášet, komentáře zůstanou navázané na starou verzi (nemažou se), nová verze začne bez nich. Migrace kotev (níže) běží jen když je přenos zapnutý.
...
* **Mapování projekt ↔ repozitář (požadavek Hany, 2026-07-10):** u projektu evidovat repozitář (URL, větev) jako přípravu na integraci. Rozdělení platforem (upřesnila Hana): **GitLab = klientské repozitáře** (kód klientů PB - sem míří mapování projektů a budoucí stažení repa přímo v aplikaci), **GitHub = nasazení ProductHubu a specifikací** (deploye HTML specifikací, repo aplikace; sem míří v2 vytváření Issues ze schválených požadavků dle zadání). Jedno pole NESTAČÍ - klient typu Headless/SFX potřebuje až 4 repa (upřesnila Hana 2026-07-27). Návrh (dvě vrstvy, kopíruje repos.yaml): (1) JÁDROVÁ sdílená repa jsou dána TYPEM klienta a definovaná JEDNOU v aplikaci - klient jen zvolí typ a jádra zdědí (nekopírují se). Tři typy (názvy dle Hany 2026-07-27): VLASTNÍ REPO (standalone/monolit - žádné sdílené jádro), OBÁLKA (MS obálka - sdílené jádro MasterShop), HEADLESS (SFX - sdílená jádra MasterShop front-api + storefront-x); (2) KLIENTSKÁ repa (obálky) = seznam per klient (URL + větev + popisek). Výsledná sada = jádra dle typu + obálky klienta. Znamená pole typu stacku + tabulku klientských repozitářů (1:N) → migrace + db-security-expert review. Plná integrace (klonování, Issues) = v2 a musí počítat s oběma platformami (GitLab klientský kód / GitHub deploye).
* **Server-side ochrana citlivých dat při sdílení klientům (požadavek Hany, 2026-07-16):** až se přes ProductHub budou sdílet dokumenty s citlivými daty (např. **analýzy návštěvnosti** konkrétního klienta), musí být na úrovni serveru zajištěno, že se citlivá data nedostanou k neoprávněnému příjemci - klient nesmí vidět data jiného klienta ani interní metriky. Rozšiřuje současný princip **server-side filtru viditelnosti** (interní vs. veřejné komentáře, M6) z komentářů na **obsah dokumentů a dat**. Řešit při návrhu sdílení analýz; MUSÍ projít `db-security-expert` review (přístupové kontroly, izolace dat mezi klienty). Pozn.: dnešní model sandboxuje obsah v iframe a chrání session, ale NEfiltruje, KDO smí vidět KTERÝ dokument nad rámec členství v projektu - jemnější klasifikace citlivosti dat je právě tento backlog.
