...
**ProductHub (interní aplikace PragueBest) - z vybraných komentářů vytvořit hotový prompt pro Claude Code, uložit ho jako „zadání" a sledovat jeho stav**
...
**Cíl:** autor vybere komentáře k zapracování, jedním klikem z nich dostane **hotový prompt** pro Claude Code, ten se **uloží jako zadání** (kdo, kdy, co) a dá se sledovat jeho stav (Vytvořeno → Předáno vývoji → Zapracováno). Autor prompt zkopíruje / stáhne a vloží do Claude Code.
...
* Vytvoření promptu z výběru - okno s náhledem (editovatelný text).
...
* AI přeformulování komentářů - prompt bere text tak, jak ho recenzent napsal.
...
4. Klik → **okno s náhledem promptu** (poskládaný markdown).
5. Autor text zkontroluje, případně **upraví přímo v okně**.
...
* Generování promptu běží v prohlížeči; **uložení** jde přes API (`POST`), stav přes API (`PATCH`) - obojí s kontrolou role a projektu.
