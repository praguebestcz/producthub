...
### Fáze 2 - Realtime (SSE) - staví se po Fázi 1

* SSE endpoint (`/api/events`), in-memory hub (Railway = 1 instance), heartbeaty proti idle timeoutu.
* Živé doručení: nový komentář se objeví v panelu bez obnovení; zvoneček naskočí živě.
* **Přítomnost:** avatary uživatelů, kteří mají dokument otevřený.
* **Indikace psaní** (volitelně).
* ⚠️ SSE se ověřuje session; při deaktivaci se stream ukončí (re-check při heartbeatu) - riziko z design docu.
* ⚠️ Živá data přes SSE MUSÍ filtrovat interní komentáře (`visibleCommentsWhere`/`canViewComment`) - jinak únik klientovi.
